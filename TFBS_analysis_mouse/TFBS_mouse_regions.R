...
match_file <- readRDS("/dcl02/hongkai/data/wzhou14/ENCODE_data/metadata/DNase_mm10_file_match.rds")
...
filelist <- list.files(path="/dcl02/hongkai/data/wzhou14/ENCODE_data/peak/mm10/DNase_house",pattern="\\.narrowPeak",full.names = TRUE, recursive = TRUE)
...
system("~/cisgenome_project/bin/refgene_getmatchedcontrol -d /users/wzhou14/cisgenome_project/mm10/annotation/refFlat_sorted.txt -dt 1 -s mouse -c /users/wzhou14/cisgenome_project/mm10/chrlen.txt -i DNase_mm10_peak_merge_250bp.cod -o DNase_mm10_peak_merge_250bp_control.cod -n 1 -l 250 -nr 1")
