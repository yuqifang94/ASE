...
age_out=GRanges()
for (i in 1:22){
  age_in=as.data.frame(read.csv(paste('../downstream/input/human_analysis/variant_age/Age_atlas/atlas.chr',i,'.csv.gz',sep=''),skip = 3))
...
saveRDS(age_out,'../downstream/input/human_analysis/variant_age/age_atlas.rds')
saveRDS(subsetByOverlaps(age_out,variant_HetCpG_meta),'../downstream/input/human_analysis/variant_age/age_atlas_sub.rds')
genonmic_1k=import('../downstream/input/human_analysis/variant_age/1000GENOMES-phase_3.gvf.gz',format='gvf')
...
saveRDS(genome_1k_variant,'../downstream/input/human_analysis/variant_age/genome_1k_variant.rds')
age_out=readRDS('../downstream/input/human_analysis/variant_age/age_atlas_sub.rds')
qual_score=0.5
...
genome_freq=readRDS('../downstream/input/human_analysis/variant_age/genome_1k_variant.rds')
...
saveRDS(variant_HetCpG_meta,"../downstream/output/human_analysis/variant_age/variant_HetCpG_meta_age.rds")
#summary analysis
...
#T-test
t.test(variant_HetCpG_meta$variant_age[variant_HetCpG_meta$dNME_pval<=pval_cutoff],
      variant_HetCpG_meta$variant_age[variant_HetCpG_meta$dMML_pval>pval_cutoff&variant_HetCpG_meta$dNME_pval>pval_cutoff])
t.test(variant_HetCpG_meta$variant_age[variant_HetCpG_meta$dMML_pval<=pval_cutoff],
      variant_HetCpG_meta$variant_age[variant_HetCpG_meta$dMML_pval>pval_cutoff&variant_HetCpG_meta$dNME_pval>pval_cutoff])      
...
t.test(variant_HetCpG_meta$variant_age[variant_HetCpG_meta$dNME_pval<=pval_cutoff&variant_HetCpG_meta$HetCpG],
  variant_HetCpG_meta$variant_age[variant_HetCpG_meta$dNME_pval<=pval_cutoff&variant_HetCpG_meta$HetCpG])
...
pdf("../downstream/output/human_analysis/variant_age/variant_age_hetCpG.pdf",width=6,height=3)
print(ggplot(all_df_HetCpG,aes(x=age,color=type))+geom_density(size=1)+xlab('variant age')+theme_glob)
#print(ggplot(all_df_HetCpG,aes(x=age,color=type))+stat_ecdf(size=1)+xlab('variant age')+theme(legend.position = 'bottom')+theme_glob)
dev.off()
# #Motif binding
...
# saveRDS(fit2.dNME,'../downstream/output/human_analysis/variant_age/edcf.dNME2.rds')
# fit2.dMML <- replicate(10000, { x <- sample(dMML_in, replace=TRUE);
# ecdf(x)(unique(dMML_in))})
# saveRDS(fit2.dMML,'../downstream/output/human_analysis/variant_age/edcf.dMML2.rds')
# fit2.non_dNME_dMML<- replicate(10000, { x <- sample(non_dNME_dMML_in, replace=TRUE);
# ecdf(x)(unique(non_dNME_dMML_in))})
# saveRDS(fit2.non_dNME_dMML,'../downstream/output/human_analysis/variant_age/edcf.non_dNME_dMML2.rds')
# cat('Finishing bootstrap',proc.time()[[3]]-tt,'\n')
# fit2.dNME=readRDS('../downstream/output/human_analysis/variant_age/edcf.dNME.rds')
# fit2.dMML=readRDS('../downstream/output/human_analysis/variant_age/edcf.dMML.rds')
# fit2.non_dNME_dMML=readRDS('../downstream/output/human_analysis/variant_age/edcf.non_dNME_dMML.rds')
...
# jpeg('../downstream/output/human_analysis/variant_age/ecdf_N1.jpg')
...
# saveRDS(fit2.dNME,'../downstream/output/human_analysis/variant_age/density.dNME.rds')
# fit2.dMML <- mcreplicate(10000, {density(sample(dMML_in, replace=TRUE),
#                                          from=min(fit1.dMML$x), to=max(fit1.dMML$x))$y},mc.cores=8)
# saveRDS(fit2.dMML,'../downstream/output/human_analysis/variant_age/density.dMML.rds')
...
# saveRDS(fit2.non_dNME_dMML,'../downstream/output/human_analysis/variant_age/density.non_dNME_dMML.rds')
# cat('Finishing bootstrap',proc.time()[[3]]-tt,'\n')

# fit2.dNME=readRDS('../downstream/output/human_analysis/variant_age/density.dNME.rds')
# fit2.dMML=readRDS('../downstream/output/human_analysis/variant_age/density.dMML.rds')
# fit2.non_dNME_dMML=readRDS('../downstream/output/human_analysis/variant_age/density.non_dNME_dMML.rds')
...
all_df=all_df[!is.na(all_df$DAF)&!is.na(all_df$age),]
pdf('../downstream/output/human_analysis/variant_age/variant_age_DAF.pdf',width=3.5,height=3.5)
ggplot(all_df,aes(x=age,color=type))+geom_density(size=1)+xlab('variant age')+theme_glob
ggplot(all_df,aes(x=DAF,color=type))+geom_density(size=1)+xlab('Derived Allele frequency (DAF)')+theme_glob
ggplot(all_df,aes(x=DAF,y=age,color=type))+geom_smooth(size=1)+xlab('DAF')+ylab("variant age")+theme_glob
dev.off()
#Age per sample 
pdf('../downstream/output/human_analysis/variant_age/variant_age_sp_density_ecdf.pdf')
...
pdf('../downstream/output/human_analysis/variant_age/ecdf_by_sample_subject.pdf')
...
dev.off()
pdf('../downstream/output/human_analysis/variant_age/ecdf_daf_by_sample_type.pdf')
...
write(genes_rank,'../downstream/output/human_analysis/variant_age/genes_rank_dNME_age_promoter.txt')
#TSS
...
write(genes_rank,'../downstream/output/human_analysis/variant_age/genes_rank_dNME_age_TSS.txt')

#Comare young and old
#Young
...
GO_young=GO_run(genes_sig,genes_all,cluster=1,mapping="org.Hs.eg.db")
#Old
genes_sig=unique(variant_HetCpG_meta$genes_promoter[variant_HetCpG_meta$variant_age>20000&variant_HetCpG_meta$dNME_pval<=pval_cutoff&variant_HetCpG_meta$N>=1])
genes_all=unique(variant_HetCpG_meta$genes_promoter[variant_HetCpG_meta$variant_age>20000&variant_HetCpG_meta$N>=1])
GO_old=GO_run(genes_sig,genes_all,cluster=1,mapping="org.Hs.eg.db")
