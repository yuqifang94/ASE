...
file_in=dir(Ken_motif_folder,pattern="OR_residual.csv")
for(fn in file_in){
  Ken_in=fread(paste0(Ken_motif_folder,fn))
  tissue=gsub("_OR_residual.csv",'',fn)
  shared_motif[[tissue]]=Ken_in[match(shared_motif$shared_motif,gsub('.*_','',Ken_in$V1))]$residual     
  
}
############End of mouse_human_covservation
############counting_ASM.R############################
