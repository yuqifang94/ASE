...
pdf('/home-4/zji4@jhu.edu/scratch/andy_ASE/MDS/UCwithP0_combine.pdf',width=7,height=6)
ggplot(pd,aes(x=MDS1,y=MDS2,col=tissue,label=time)) + geom_text(show.legend = F) + geom_point(size=0,stroke=0) + theme_classic() + scale_color_manual(values=cv) + theme(legend.title = element_blank()) + guides(colour = guide_legend(override.aes = list(size=3,stroke=1)))
dev.off()
