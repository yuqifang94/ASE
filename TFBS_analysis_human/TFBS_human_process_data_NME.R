...
data_median_DNase <- lapply(c(1:3), function(x) {
	...
	data_temp <- readRDS(glue("/dcl02/hongkai/data/wzhou14/Andy_lab/share/NME_motif_DNase_hg19/JASPAR_motif_hg19_NME_{x}_agnostic_DNase.rds"))
...
data_median_control <- lapply(c(1:3), function(x) {
	...
	data_temp <- readRDS(glue("/dcl02/hongkai/data/wzhou14/Andy_lab/share/NME_motif_DNase_hg19/JASPAR_motif_hg19_NME_{x}_agnostic_Control.rds"))
	data_temp_median <- sapply(data_temp, function(y){
		data_mat <- as.matrix(mcols(y))
		data_mat_median <- colMedians(data_mat,na.rm = TRUE)
		names(data_mat_median) <- colnames(data_mat)
